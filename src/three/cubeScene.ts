...
import {
  ...
  PEEK_MS,
  PLASTIC_COLOR,
  PREVIEW_ARMED_EMISSIVE,
  PREVIEW_DIM_EMISSIVE,
  STICKER_COLORS,
  TURN_MS,
} from './constants'
import { createFifoQueue, interpolateAngle } from './effects'
import { axisUnit, clamp, dominantAxisIndex, easeInOutCubic } from './math'
import { moveForAxisLayer, parseMove } from './moves'
import { peekTargetFor } from './view'
import type {
  ActivePeekTween,
  ActivePreview,
  ActiveTween,
...
export type { CubeSceneOptions, PeekFace } from './types'
export type { CubeSize, FaceName } from './types'
export { FACE_ORDER, SCRAMBLE_TURN_MS } from './constants'
export { createSolvedStickers } from './moves'

/**
 * Isolated three.js Rubik's Cube renderer — a dumb view with no game logic.
 * (docs moved to modules; behavior unchanged)
 */
...
export class CubeScene {
  ...
  private queue = createFifoQueue<QueueEntry>()
  ...
  playMove(move: string, ms: number = TURN_MS): Promise<void> {
    if (this.disposed) {
      return Promise.reject(new Error('CubeScene is disposed'))
    }
  ...
  reset(stickers: number[][], n: number): void {
    ...
    const dropped = this.queue.spliceAll()
  ...
  private stickerMaterial(colorIndex: number): THREE.MeshStandardMaterial {
    ...
    let material = key === undefined ? undefined : this.materialCache.get(key)
    if (!material) {
  ...
  private pump(now: number): void {
    if (!this.tween && !this.queue.isEmpty()) {
      const next = this.queue.shift()
      if (next) {
        this.startTween(next, now)
      }
    }
    const active = this.tween
    if (active) {
      const t = Math.min((now - active.t0) / active.dur, 1)
      active.pivot.setRotationFromAxisAngle(active.axis, interpolateAngle(active.from, active.angle, t))
      if (t >= 1) {
        this.finishTween()
      }
    }
  ...
  private isBusy(): boolean {
    return this.tween !== null || this.preview !== null || !this.queue.isEmpty()
  }
  ...
  peek(face: PeekFace | null): void {
    ...
    if (this.tween !== null || this.preview !== null || !this.queue.isEmpty() || this.current !== null) {
  ...
  private pick(event: PointerEvent | MouseEvent): GrabHit | null {
    ...
    if (localNormal.dot(snappedLocal) < 0) {
      snappedLocal.negate()
    }
  ...
  private onPointerDown = (event: PointerEvent): void => {
    if (!this.interactive || this.disposed) {
      return
    }
    if (event.button === 1) {
  ...
  private onPointerMove = (event: PointerEvent): void => {
    ...
    if (!gesture || event.pointerId !== gesture.pointerId || gesture.decided === 'done') {
      return
    }
    ...
    if (Math.hypot(event.clientX - gesture.startX, event.clientY - gesture.startY) < DRAG_DECIDE_PX) {
      return
    }
    ...
    if (this.tryEnterPreview(gesture, event)) {
      return
    }
    gesture.decided = 'done'
    gesture.lastX = event.clientX
  ...
  private restorePreviewMaterials(preview: ActivePreview): void {
    preview.members.forEach((member, i) => {
      ...
      for (const mat of curs) {
        mat.dispose()
      }
      const saved = preview.savedMats[i]
      if (saved !== undefined) {
        member.material = saved
      }
    })
  }
  ...
  private updatePreview(gesture: PendingGesture, event: PointerEvent): void {
    ...
    if (v0.lengthSq() < 1e-8 || v1.lengthSq() < 1e-8) {
      return
    }
    v0.normalize()
    v1.normalize()
